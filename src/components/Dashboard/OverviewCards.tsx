          ...
          {onboarding.map((step, i) => {
            ...
            return (
              ...
              >
                ...
                <ArrowRight size={18} className="text-slate-300 group-hover:text-emerald-500 shrink-0 transition-colors" />
              </Link>
            );
          })}
        </div>
      ...
      {nextExam && nextCountdown && !nextCountdown.isOverdue && (
        <div className="card rounded-2xl p-5 border border-orange-200 bg-orange-50 shadow-sm col-span-1 md:col-span-2 lg:col-span-1">
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <h3 className="font-bold text-orange-900 text-sm">{nextExam.subjectName}</h3>
          </div>
          ...
          <ul className="space-y-3 mt-3">
            {todaySlots.map((slot) => (
              <li key={slot.id} className="flex justify-between items-center text-sm">
                <span className="font-semibold text-slate-700 truncate mr-4">{slot.subjectName}</span>
                <span className="font-medium text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100 shrink-0">{formatRange(slot)}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-slate-500 text-sm mt-3 font-medium">Không có lịch học hôm nay</p>
        )}
      </div>
      ...
      {nearestMilestone && (
        <div className="card rounded-2xl p-5 border border-blue-200 bg-blue-50 shadow-sm md:col-span-2 lg:col-span-4">
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <p className="text-sm font-semibold text-blue-600/80 uppercase tracking-wide bg-blue-100 px-2.5 py-1 rounded-lg">Hạn: {nearestMilestone.deadlineDate}</p>
          </div>
        </div>
      )}
    </div>
