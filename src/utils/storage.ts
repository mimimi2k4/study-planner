import type {
  ExamInfo, Syllabus, FreeSlot, StudyTask, StudyPlan, ScheduleSlot,
} from '../types'
...
export function executePlanAction(
  ...
  if (action === 'move_task') {
    const { slotId, newDate, newStartTime, newEndTime } = payload as {
      slotId: string; newDate: string; newStartTime: string; newEndTime: string
    }
    if (!slotId || !newDate) return { success: false, error: 'Thiếu thông tin dịch chuyển' }
    plan.slots = plan.slots.map((s): ScheduleSlot =>
      s.id === slotId ? { ...s, date: newDate, startTime: newStartTime, endTime: newEndTime, manualEdited: true } : s
    )
    plan.manualEdited = true
    savePlan(plan)
    return { success: true }
  ...
  if (action === 'update_task_status') {
    ...
    saveTasks(tasks.map((t) => (t.id === taskId ? { ...t, status: status as StudyTask['status'] } : t)))
    return { success: true }
  }
