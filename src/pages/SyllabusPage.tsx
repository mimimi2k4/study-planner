...
export default function SyllabusPage({ syllabuses, exams, onAdd, onUpdate, onDelete }: SyllabusPageProps) {
  ...
  function handleAdd(state: SyllabusFormState) {
    ...
    onAdd({ id: nanoid(), subjectId: state.subjectId, subjectName: exam.subjectName, chapters: state.chapters })
  }

  function handleUpdate(id: string, state: SyllabusFormState) {
    const exam = exams.find((e) => e.id === state.subjectId)!
    onUpdate({ id, subjectId: state.subjectId, subjectName: exam.subjectName, chapters: state.chapters })
  }
